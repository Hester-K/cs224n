...
def naiveSoftmaxLossAndGradient(
    ...
    y_o = np.array([0 if i != outsideWordIdx else 1 for i in range(len(outsideVectors))])
    y_hat = softmax(outsideVectors @ centerWordVec)
    loss = - np.log(y_hat[outsideWordIdx])
    gradCenterVec = np.transpose(outsideVectors) @ (y_hat - y_o)
    gradOutsideVecs = np.outer(y_hat - y_o, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    uv = outsideVectors @ centerWordVec
    sig_idx = [sigmoid(uv[indices[0]])] + [sigmoid(- uv[indices[i]]) for i in range(1, len(indices))]
    u_idx = [outsideVectors[indices[0]]] + [outsideVectors[indices[i]] for i in range(1, len(indices))]

    loss = -np.log(sig_idx[0]) - np.sum([np.log(sig_idx[i]) for i in range(1, len(indices))])

    gradCenterVec += - np.multiply(1 - sig_idx[0], u_idx[0]) 
    gradOutsideVecs[indices[0]] += - np.multiply(1 - sig_idx[0], centerWordVec)
    for i in range(1, len(indices)):
        gradCenterVec += np.multiply(1 - sig_idx[i], u_idx[i])
        gradOutsideVecs[indices[i]] += np.multiply(1 - sig_idx[i], centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    v_c = centerWordVectors[centerWordIdx]
    for outsideWord in outsideWords:
        idx_loss, idx_grad1, idx_grad2 = word2vecLossAndGradient(v_c, word2Ind[outsideWord], outsideVectors, dataset)
        loss += idx_loss
        gradCenterVecs[centerWordIdx] += idx_grad1
        gradOutsideVectors += idx_grad2
    ### END YOUR CODE
