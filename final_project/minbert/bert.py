...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    att_scores = att_scores / math.sqrt(self.attention_head_size)
    att_scores = att_scores.masked_fill(attention_mask < 0, -1e9)
    att_scores = F.softmax(att_scores, dim=-1)
    att_scores = torch.matmul(att_scores, value)
    att_scores = att_scores.permute(0, 2, 1, 3)
    att_scores = att_scores.reshape(att_scores.shape[0], att_scores.shape[1], self.all_head_size)
    return att_scores
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    temp1 = self.add_norm(hidden_states, att, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    temp2 = self.interm_af(self.interm_dense(temp1))
    temp3 = self.add_norm(temp1, temp2, self.out_dense, self.out_dropout, self.out_layer_norm)
    return temp3
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    embed = self.embed_dropout(self.embed_layer_norm(embed))
    return embed
