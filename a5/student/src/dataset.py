...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = [self.stoi[c] for c in self.data[idx]]
        truncated = document[0: random.randint(4, int(self.block_size * 7 / 8))]
        
        mask_mid_point = random.randint(int(len(document)/4), int(3 * len(document)/4))
        mask_half_length = random.randint(0, int(len(document)/4))
        # print(mask_half_length, mask_mid_point)
        prefix = truncated[0: mask_mid_point - mask_half_length]
        masked_content = truncated[mask_mid_point - mask_half_length: mask_mid_point + mask_half_length]
        suffix = truncated[mask_mid_point + mask_half_length:]
        # print(prefix, masked_content, suffix)
        masked_string = prefix + [self.stoi[self.MASK_CHAR]] + suffix + [self.stoi[self.MASK_CHAR]] + masked_content
        masked_string = masked_string + [self.stoi[self.PAD_CHAR]] * (self.block_size - len(masked_string))
        masked_output = masked_string[1:] + [self.stoi[self.PAD_CHAR]]

        x = torch.tensor(masked_string, dtype=torch.long)
        y = torch.tensor(masked_output, dtype=torch.long)
        return (x, y)
